...
const files = ['frontend/src/components/sidebar/AppSidebar.jsx'];
...
const tarPath = path.join(os.tmpdir(), `himjourney-sidebar-nav-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-sidebar-nav.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-sidebar-nav.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-sidebar-nav.tgz',
