...
const files = [
  'frontend/src/components/lead-detail/LeadDetailHeader.jsx',
  'frontend/src/components/lead-detail/LeadDetailLayout.jsx',
  'frontend/src/components/lead-detail/LeadOverviewSection.jsx',
  'frontend/src/features/leads/hooks/useLeadActivities.js',
  'frontend/src/hooks/useLeadAssign.js',
  ...
  'frontend/src/components/team-leader/LeaderLeadDetailPage.jsx',
];
...
const tarPath = path.join(os.tmpdir(), `himjourney-lead-mockup-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-lead-mockup.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-lead-mockup.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-lead-mockup.tgz',
