...
const tarPath = path.join(os.tmpdir(), `himjourney-footer-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-footer.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-footer.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-footer.tgz',
