...
const files = [
  'backend/src/constants/leadSources.js',
  ...
  'backend/src/services/leadAnalyticsService.js',
  'backend/src/services/publicLeadIngestService.js',
  'backend/src/services/facebookLeadWebhookService.js',
  ...
  'frontend/src/components/dashboard/LeadSourceChart.jsx',
  'frontend/src/components/dashboard/LeadSourcePerformanceTable.jsx',
  'frontend/src/components/sales-executive/dashboard/ExecutiveLeadSourceChart.jsx',
  'frontend/src/components/sales-manager/LeadListBadges.jsx',
  'frontend/src/components/quotations/MobileQuotationDetail.jsx',
  'frontend/src/components/lead-detail/leadDetailUtils.js',
];
...
const tarPath = path.join(os.tmpdir(), `himjourney-sources-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-sources.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-sources.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-sources.tgz',
