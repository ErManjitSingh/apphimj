...
const tarPath = path.join(os.tmpdir(), `himjourney-kpi-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-kpi.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-kpi.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-kpi.tgz',
