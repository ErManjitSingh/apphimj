...
const tarPath = path.join(os.tmpdir(), `himjourney-hotel-ui-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-hotel-ui.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-hotel-ui.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-hotel-ui.tgz',
