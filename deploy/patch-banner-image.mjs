...
const tarPath = path.join(os.tmpdir(), `himjourney-banner-${Date.now()}.tgz`);
execLocal('tar.exe', ['-C', REPO, '-czf', tarPath, ...files]);

const conn = await connect();
try {
  await sftpPut(conn, tarPath, '/tmp/himjourney-banner.tgz');
  await exec(
    conn,
    [
      `tar -xzf /tmp/himjourney-banner.tgz -C ${APP}`,
      'rm -f /tmp/himjourney-banner.tgz',
