...
function sparkPath(values = []) {
  const points =
    values.length > 1
      ? values
      : [0, Number(values[0] || 0), 0, Number(values[0] || 0)];
...
function QuickAction({
  ...
  const children = (
    <>
      ...
      <span
        className={`text-[9px] font-medium ${primary ? "text-white" : "text-slate-700"}`}
      >
        {label}
      </span>
    </>
...
export default function MobileAdminDashboard({
  ...
  const sourceTotal =
    sources.reduce((sum, row) => sum + Number(row.value || 0), 0) ||
      ...
      <section className="relative min-h-[222px] overflow-hidden rounded-b-[24px] bg-[#070b48] px-5 pb-10 pt-4 text-white">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <button
            ...
            className="rounded-xl p-1 text-white/90"
            aria-label="Open menu"
          >
            <Menu className="h-5 w-5" />
          </button>
          <div className="flex items-center gap-3">
            <Link to="/leads" aria-label="Search leads">
            ...
            <Link
              to="/profile"
              className="relative flex h-8 w-8 items-center justify-center rounded-full bg-violet-500 text-[9px] font-bold"
            >
              {initials(user?.name)}
      ...
      <div className="-mt-5 px-5">
        <div className="relative z-10 flex min-h-[64px] items-center justify-between rounded-2xl bg-white px-3 py-2 shadow-[0_10px_30px_rgba(32,25,92,0.14)]">
          <QuickAction
            ...
            label="Import"
            tone="text-violet-600"
            onClick={() => toast.info("Lead CSV import is being prepared.")}
          />
          <QuickAction
            icon={Download}
            label="Export"
            tone="text-emerald-600"
            onClick={() => exportRecentLeads(recentLeads)}
          />
            ...
            <div className="flex gap-2">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    onFiltersChange({ ...filters, ...presetRange(key) })
                  }
        ...
        {isFetching && (
          <div className="mt-3 h-0.5 overflow-hidden rounded-full bg-violet-100">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {KPI_CONFIG.map((card) => {
            ...
            const wrapperProps = card.path
              ...
              : {
                  className:
                    "min-w-0 rounded-2xl border border-slate-100 bg-white p-2.5 shadow-sm",
                };
            return (
              <Wrapper key={card.key} {...wrapperProps}>
                <div className="flex items-start gap-1.5">
                  <span
                    className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br text-white ${card.tone}`}
                  >
                    <Icon className="h-3.5 w-3.5" />
                  </span>
                  <div className="min-w-0">
                    <p className="truncate text-[7px] font-bold uppercase tracking-wide text-slate-500">
                      {card.label}
                    </p>
                ...
                <div className="mt-1">
                  <KpiSparkline
                    values={stats?.kpiSparklines?.[sparkKey] || []}
                    color={card.color}
                  />
                </div>
              </Wrapper>
        ...
        <section className="mt-3 rounded-2xl border border-slate-100 bg-white p-3 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-lg border border-slate-200 px-2 py-1 text-[8px] font-semibold text-slate-600">
              This Month⌄
            </span>
          </div>
          <div className="mt-3 flex gap-2">
            <div className="w-[78px] shrink-0 space-y-3">
              <div>
                ...
                <p className="text-[8px] text-violet-600">This Month</p>
              </div>
              <div>
                <p className="text-xs font-bold text-slate-600">
                  {formatCurrency(previousRevenue)}
                </p>
                <p className="text-[8px] text-slate-400">Last Month</p>
              </div>
              <div>
                <p
                  className={`text-sm font-bold ${growth < 0 ? "text-rose-500" : "text-emerald-600"}`}
                >
                  {growth < 0 ? "↓" : "↑"} {Math.abs(growth).toFixed(1)}%
                </p>
                <p className="text-[8px] text-slate-400">Growth</p>
              </div>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ borderRadius: 10, fontSize: 9 }}
                  />
                  <Area
                    type="monotone"
                    dataKey="previous"
        ...
        <section className="mt-3 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
            ...
            <Link
              to="/leads"
              className="text-[8px] font-semibold text-violet-600"
            >
              View All
            </Link>
          </div>
          <div className="divide-y divide-slate-100">
            {recentLeads.slice(0, 5).map((lead, index) => (
              ...
              >
                <span
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[8px] font-bold text-white ${
                    [
                      "bg-violet-500",
                      "bg-emerald-500",
                      "bg-orange-500",
                ...
                <div className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[7px] font-semibold ${STATUS_STYLES[lead.status] || "bg-slate-100 text-slate-600"}`}
                    >
            ...
            {!recentLeads.length && (
              <p className="px-4 py-8 text-center text-[10px] text-slate-400">
                No recent leads
              </p>
            )}
          </div>
