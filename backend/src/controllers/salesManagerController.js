...
const updateQuotation = asyncHandler(async (req, res) => {
  ...
  if (action === 'approve') {
    ...
    resolvePendingDiscountEntries(quotation, {
      status: 'approved',
      actor: req.user,
      note: 'Extra discount approved by Sales Manager',
    });
    quotation.timeline.push({
      ...
      notes:
        notes ||
        `Approved by Sales Manager · Costing 2 @ ${costing2.markupPercent}% (₹${Number(costing2.grandTotal).toLocaleString('en-IN')})`,
    });
    ...
    resolvePendingDiscountEntries(quotation, {
      status: 'rejected',
      actor: req.user,
      note: notes || 'Extra discount rejected by Sales Manager',
    });
    quotation.timeline.push({
      type: 'rejected',
      date: now,
